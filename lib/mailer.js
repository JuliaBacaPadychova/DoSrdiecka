...
// V hlavičkách e-mailu smie byť podľa normy len obyčajná anglická abeceda.
// Čokoľvek s diakritikou treba zakódovať — inak to príjemcovi buď vyjde
// ako "Do srdieÄka", alebo mu server pripočíta body v hodnotení spamu.
// Týka sa to aj mena odosielateľa, nielen predmetu.
function encodeHeader(text) {
  const cisteAscii = /^[\x20-\x7E]*$/.test(text);
  return cisteAscii ? text : `=?UTF-8?B?${b64(text)}?=`;
}
...
async function sendMail({ to, from, subject, text, replyTo }) {
  ...
  try {
    ...
    const headers = [
      `From: ${encodeHeader("Do srdiečka")} <${user}>`,
      `To: <${to}>`,
...
module.exports = { sendMail, SmtpError, encodeHeader };
