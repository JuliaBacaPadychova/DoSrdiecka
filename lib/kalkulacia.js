...
function davkyReceptu(recept, vazba, pocet) {
...
function potrebaSurovin(polozky, data) {
  ...
  for (const { product_id, kusy } of polozky) {
    ...
    for (const vazba of vazby.get(product_id) || []) {
      ...
      const podiel = davkyReceptu(recept, vazba, pocet);
...
function rozpisReceptov(polozky, data) {
  ...
  for (const { product_id, kusy } of polozky) {
    ...
    for (const vazba of vazby.get(product_id) || []) {
      const recept = recepty.get(vazba.recipe_id);
      if (!recept) continue;
      const davky = davkyReceptu(recept, vazba, pocet);
  ...
  return [...davkyPodlaReceptu.values()].map(({ recept, davky, kusy, vytazok, kusov_z_davky }) => ({
    recept: {
      id: recept.id,
      nazov: recept.name,
      druh: recept.kind,
