...
function koeficientTorty(recept, vazba, kontext) {
  ...
  // Na koľko vrstiev je recept napísaný a koľko ich chcem. Prázdne je
  // jedna vrstva, nie nula — recept bez vrstiev (ganache na obter) sa
  // riadi len plochou.
  const naRecept = cislo(recept.layers) || 1;
    ...
    for (const vazba of vazby.get(product_id) || []) {
      ...
      const naTortu = koeficientTorty(recept, vazba, kontext) !== null
        ? {
          ...
          vrstiev: (kontext.vrstvy ? cislo(kontext.vrstvy[recept.id]) : null)
            ?? cislo(vazba.layers) ?? cislo(recept.layers) ?? 1,
        }
        : null;
