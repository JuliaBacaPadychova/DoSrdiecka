...
async function getAdminUser(req) {
  ...
  // Chybu z authGetUser zámerne NEZACHYTÁVAME: "token je neplatný" vráti
  // null, "Supabase sa neozval" vyhodí chybu. Keby sme to zliali dokopy,
  // výpadok by vyzeral ako odhlásenie.
  return authGetUser(token);
}
...
function requireAdmin(handler) {
  return async (req, res) => {
    ...
    try {
      ...
      // Overenie sa nepodarilo dokončiť. To neznamená, že je majiteľka
      // odhlásená — správa webu ju pri 503 na prihlásenie nevyhodí.
      // eslint-disable-next-line no-console
      console.error("Overenie prihlásenia zlyhalo:", err.message);
      res.status(503).json({
        error: "auth_unavailable",
        message: "Databáza sa teraz neozýva. Skús to prosím o chvíľu znova.",
      });
      return;
    }
    if (!user) {
